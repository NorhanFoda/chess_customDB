function wk_pos_value(game, pos, defenders, attackers){
    
    //check if white king is castled
    var casteled = 0;
    var history = game.history();
    if(history[(game.history()).length-2] == 'O-O' || history[(game.history()).length-2] == 'O-O-O'){
        casteled = 1;
    }
    ...
    for(var i = letters.indexOf(col)-3; i <= letters.indexOf(col)+3; i++){
        ...
        else if(p != null && p.color == 'b' && p.type == 'p'){
